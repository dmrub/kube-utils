...
def main():
    ...
    for pod in pod_list.items:
        ...
        for container in pod.spec.containers:
            ...
            command = 'nvidia-smi --query-compute-apps=gpu_name,gpu_bus_id,pid,process_name,used_memory --format=csv,noheader'
            ...
            try:
                ...
                if rc == 0:
                    print('Pod {} in namespace {} on node {} uses GPU'.format(pod_name, pod_namespace, pod_node_name))
                    f = StringIO(stdout)
                    ...
                    command = 'sh -c \'for p in /proc/*; do if [ -e "$p/cmdline" ]; then printf "%s\\t%s\\n" "$p" "$(' \
                              'tr \\\\0 " " < "$p/cmdline")"; fi; done\' '
                    ...
                    if rc == 0:
                        ...
                        key = '{}|{}|{}|{}|{}'.format(GPU_CHECK_PROC_PREFIX, pod_name, pod_namespace, pod_node_name,
                                                      container.name)
    ...
    for gpu_node in gpu_node_to_containers_map.keys():
        ...
        node_pod_name = 'x-gpuproc-' + gpu_node
            ...
            for line in stdout.splitlines(keepends=False):
                ...
                if GPU_CHECK_PROC_PREFIX in cmdline:
                    ...
                    if container_key is None:
                        raise Exception('Internal error, could not parse parameter with prefix {} from command line {}'
